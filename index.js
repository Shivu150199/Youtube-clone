...
const apiKey = 'AIzaSyD844UJTeuqWkFc75khF0ld-IYWWK0EYRo'
...
async function randerVideos(videos) {
    let videoContainer = document.querySelector('.right');
...
async function createVideoCard(e, videoId) {
  try {
    ...
    videoCard.innerHTML = `
            ...
            <div class="video-details">
                ...
                <div class="video-name-views">
                    <p class="name">${videoTitle
                      .split('')
                      .slice(0, 25)
                      .join('')}${
      videoTitle.split('').length > 25 ? '...' : ''
    }</p>
                    <p class="ch-name">
                        ${channelTitle}
                        <br>
                        ${formatViewCount(
                          viewCount
                        )} Views . ${formatPastDuration(publishTime)} ago
                    </p>
                </div>
            </div>
        `
    ...
    videoCard.addEventListener('mouseenter', (e) =>
      playVideoPreview(videoCard, videoId, channelId)
    )
    videoCard.addEventListener('mouseleave', (e) =>
